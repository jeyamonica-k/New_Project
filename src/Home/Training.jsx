...
function Training() {
  const [clickedIndexes, setClickedIndexes] = useState([false, false, false, false]);
      ...
      <div className="flex flex-col items-center gap-5 p-2 m-2 lg:m-0  lg:flex-row lg:mb-18 lg:mx-12">
        {values.map((data, index) => {
          return (
            <div
              key={index}
              className={`max-w-full relative w-[300px] h-[400px] md:w-[500px] md:h-[600px] lg:w-[450px] lg:h-[400px] group overflow-hidden ${index === 1 || index === 3 ? "lg:mt-20" : ""}`}
            ...
            >
              ...
              <img
                src={data.src}
                alt="Example"
                className={`w-full h-full object-cover transition duration-300  ${clickedIndexes[index] ? "scale-105 blur-[2px]" : ""}`}
              />

              {/* Top Red Diagonal Covering Left Top Corner */}
              <div
                className={`absolute top-0 left-0 w-full h-[15%] bg-red-600 transform -skew-y-18 origin-top-left transition-all duration-300 scale-0 ${clickedIndexes[index] ? "opacity-100 scale-100" : ""}`}
              ></div>

              {/* Bottom Blue Diagonal from Bottom-Left to Top-Right */}
              <div
                className={`absolute bottom-0 left-0 w-full h-1/2 bg-blue-950 transform -skew-y-11 origin-bottom-right transition-all duration-300 scale-0 ${clickedIndexes[index] ? "opacity-100 scale-100" : ""}`}
              ></div>

              {/* Paragraph positioned between image and bottom slanted blue line */}
              <div
                className={` transition-all duration-300 ${clickedIndexes[index] ? "opacity-100 " : "opacity-0"} flex-col gap-1 absolute bottom-0 left-0  p-1 m-1 `}
              >
